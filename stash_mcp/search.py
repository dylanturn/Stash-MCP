...
# Maximum document size in characters for contextual retrieval prompts.
# Prevents exceeding the model context window (~200k tokens).
_MAX_CONTEXT_DOC_CHARS = 150_000
...
class SearchEngine:
    ...
    def __init__(
        ...
        *,
        ...
        # If embedder model changed, refuse to serve stale results
        if self.meta.embedder_model and self.meta.embedder_model != embedder_model:
            logger.warning(
                ...
                f"to '{embedder_model}'. Index is stale — reindex required."
            )
            self._ready = False
        else:
    ...
    def _init_embedder(self):
        """Lazily initialise the Pydantic AI Embedder."""
    ...
    async def _embed(self, texts: list[str]) -> list[list[float]]:
        ...
        Uses ``embed_documents`` for document content during indexing.
    ...
    async def _embed_query(self, text: str) -> list[float]:
        ...
        Uses ``embed_query`` which some providers optimise differently
        from ``embed_documents`` (e.g. Cohere applies different prefixes).
    ...
    async def _contextualise_chunk(
        ...
        try:
            ...
            # Truncate document to avoid exceeding the model context window
            doc_text = full_document[:_MAX_CONTEXT_DOC_CHARS]

            contextual_model = os.getenv(
                "STASH_CONTEXTUAL_MODEL", "claude-haiku-4-5-20251001"
            )

            prompt = (
                f"<document>{doc_text}</document>\n"
    ...
    async def reindex(self) -> int:
        ...
        Uses the FileSystem instance for file discovery when available,
        respecting ``STASH_CONTENT_PATHS`` filtering.  Falls back to
        ``content_dir.rglob`` when no FileSystem is provided.
        ...
        return await self.build_index(file_paths)
