...
| Tool                       | Implementation                                                                                                                                                                                                                                   |
| -------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `read_content`             | `StorageBackend.read(store_id, path, at_commit, at_event)`. The backend issues a single `tree_entries` lookup and an S3 GET, returning a `ReadResult` with content + blame metadata. Optional `at_commit` pins to a historical commit; optional `at_event` pins to the state immediately after a given event. |
| `read_content_batch`       | `StorageBackend.read_batch(store_id, paths, at_commit, at_event)`. The backend resolves all paths against a single commit (one `tree_entries` query joining the paths array) and fans out blob fetches in parallel. Returns `dict[path, ReadResult]`; absent paths are omitted. |
...
| `create_content` / `edit_content` | Build `EventDescriptor`s (`kind` inferred from intent: `created` if path absent, `replaced` for whole-file rewrite, `patched` if a structured patch is supplied). The existing `expected_sha256` parameter maps one-for-one to `EventDescriptor.expected_before_sha`; a mismatch surfaces as `ConflictError`. Call the commit protocol. Optional params: `semantic_summary`, `patch`. |
